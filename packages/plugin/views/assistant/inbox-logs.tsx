...
const LogEntryDisplay: React.FC<{ entry: LogEntry; step: Action }> = ({
  ...
  return (
    <div className="flex items-center gap-2 py-1.5">
      ...
      {/* Timestamp */}
      <span className="text-[var(--text-dim)] w-20 text-xs opacity-60">
      ...
      {entry.error && (
        <div
          className="flex items-center gap-1 text-[var(--text-sub-accent)] text-sm ml-auto"
...
const FileNameDisplay: React.FC<{ record: FileRecord }> = ({ record }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[var(--text-accent)]">{record.newName}</span>
    </div>
  );
};
...
const PathDisplay: React.FC<{ record: FileRecord }> = ({ record }) => {
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      <span className="text-[var(--text-dim)]">→</span>
      <span className="text-[var(--text-accent)]">{record.newPath}</span>
    </div>
  );
};
    ...
    <div className="space-y-2">
      {hasRename && (
        <div className="text-sm">
          Renamed as{" "}
          <span
            className="text-[var(--text-accent)] cursor-pointer hover:underline"
            onClick={() => {
              if (record.file) {
          ...
          <span className="text-[var(--text-dim)] text-xs opacity-50">
...
function FileCard({ record }: { record: FileRecord }) {
  ...
  return (
    <div className="border-b border-[var(--border-subtle)] hover:bg-[rgba(14,210,247,0.06)] transition-colors duration-150">
      <div
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            {hasExpandableContent && (
              <div className="flex-shrink-0">
                {isExpanded ? (
                  <ChevronDown className="w-4 h-4 text-[var(--text-accent)]" />
                ) : (
            ...
            >
              ...
              {errorSummary && (
                <span className="text-[var(--text-sub-accent)] text-xs truncate">
                  • {errorSummary}
                </span>
              )}
            </div>
...
const InboxAnalytics: React.FC<{
  ...
  const StatusBox = ({
    ...
    <div
      key={status}
      className="bg-[var(--bg-depth-1)] p-4 rounded text-center flex flex-col items-center border border-[var(--border-subtle)]"
    >
      <div className="text-sm capitalize">{status}</div>
      <div className="font-semibold">{byStatus[status] || 0}</div>
      <div className="mt-1 text-[var(--text-dim)]">{icon}</div>
    </div>
  );

  return (
    <div className="bg-[var(--bg-depth-2)] h-full">
      <div className="space-y-2">
        {/* Main flow row */}
    ...
    <div className="flex items-center gap-2">
      <div className="relative flex items-center gap-2">
        ...
        <select
          ...
          className="pl-2 pr-8 h-min py-2 bg-[var(--bg-depth-3)] text-[var(--text-normal)] rounded-l border border-[var(--border-defined)] text-sm appearance-none"
        >
      ...
      {value.range === "custom" && (
        <input
          ...
          className="py-2 pl-6 pr-2 bg-[var(--bg-depth-3)] text-[var(--text-normal)] rounded-r border border-[var(--border-defined)] text-sm w-min"
...
const SearchBar: React.FC<SearchBarProps> = ({
  ...
  return (
    <div className="bg-[var(--bg-depth-1)] p-4 border border-[var(--border-defined)] rounded space-y-3">
      {/* Search input row */}
      <div className="pl-10 relative flex-1">
        <Search className="w-4 h-4 absolute left-3 top-1/2 transform -translate-y-1/2 text-[var(--text-accent)]" />
        <input
          type="text"
          placeholder="Search files, tags, or actions..."
          value={searchQuery}
          onChange={handleSearchChange}
          className="w-full pl-10 pr-4 h-min py-2 bg-[var(--bg-depth-3)] text-[var(--text-normal)] rounded border border-[var(--border-defined)] text-sm placeholder-[var(--text-dim)]"
        />
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative w-[200px]">
          <Filter className="w-4 h-4 absolute left-3 top-1/2 transform -translate-y-1/2 text-[var(--text-accent)]" />
          <select
            value={selectedStatus}
            onChange={e => onStatusFilter(e.target.value as FileStatus | "")}
            className="w-full pl-9 pr-4 h-min py-2 bg-[var(--bg-depth-3)] text-[var(--text-normal)] rounded border border-[var(--border-defined)] text-sm appearance-none"
          >
            {statuses.map(status => (
...
export const InboxLogs: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {dateFilter.range !== "all" && (
        <div className="text-xs text-[var(--text-dim)] px-3 py-1 border-b border-[var(--border-subtle)]">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {filteredRecords.length === 0 && (
          <div className="flex items-center justify-center h-32 text-xs text-[var(--text-dim)] opacity-50">
