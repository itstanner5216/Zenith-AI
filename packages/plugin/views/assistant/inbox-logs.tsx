...
const LogEntryDisplay: React.FC<{ entry: LogEntry; step: Action }> = ({
  ...
  return (
    <div className="flex items-center gap-2 py-1.5">
      ...
      {/* Timestamp */}
      <span className="text-[#45aaff] w-20 text-xs opacity-60">
      ...
      {entry.error && (
        <div
          className="flex items-center gap-1 text-[#f4569d] text-sm ml-auto"
    ...
    <div className="space-y-2">
      {hasRename && (
        <div className="text-sm">
          Renamed as{" "}
          <span
            className="text-[#0fb6d6] cursor-pointer hover:underline"
            onClick={() => {
              if (record.file) {
          ...
          <span className="text-[#45aaff] text-xs opacity-50">
...
function FileCard({ record }: { record: FileRecord }) {
  ...
  return (
    <div className="border-b border-[rgba(14,210,247,0.05)] hover:bg-[rgba(14,210,247,0.06)] transition-colors duration-150">
      <div
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            >
              ...
              {errorSummary && (
                <span className="text-[#f4569d] text-xs truncate">
                  • {errorSummary}
                </span>
              )}
            </div>
...
const InboxAnalytics: React.FC<{
  ...
  const StatusBox = ({
    ...
    <div
      key={status}
      className="bg-[#0d0b12] p-4 rounded text-center flex flex-col items-center border border-[rgba(14,210,247,0.05)]"
    >
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="relative flex items-center gap-2">
        <Calendar className="w-4 h-4 text-[#0fb6d6]" />
        <select
          value={value.range}
          onChange={e => handleRangeChange(e.target.value as DateRange)}
          className="pl-2 pr-8 h-min py-2 bg-[#191621] text-[#bebebe] rounded-l border border-[rgba(14,210,247,0.08)] text-sm appearance-none"
        >
      ...
      {value.range === "custom" && (
        <input
          ...
          className="py-2 pl-6 pr-2 bg-[#191621] text-[#bebebe] rounded-r border border-[rgba(14,210,247,0.08)] text-sm w-min"
...
const SearchBar: React.FC<SearchBarProps> = ({
  ...
  return (
    <div className="bg-[#0d0b12] p-4 border border-[rgba(14,210,247,0.08)] rounded space-y-3">
      {/* Search input row */}
      <div className="pl-10 relative flex-1">
        <Search className="w-4 h-4 absolute left-3 top-1/2 transform -translate-y-1/2 text-[#0fb6d6]" />
        <input
          type="text"
          placeholder="Search files, tags, or actions..."
          value={searchQuery}
          onChange={handleSearchChange}
          className="w-full pl-10 pr-4 h-min py-2 bg-[#191621] text-[#bebebe] rounded border border-[rgba(14,210,247,0.08)] text-sm placeholder-[#45aaff]"
        />
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative w-[200px]">
          <Filter className="w-4 h-4 absolute left-3 top-1/2 transform -translate-y-1/2 text-[#0fb6d6]" />
          <select
            value={selectedStatus}
            onChange={e => onStatusFilter(e.target.value as FileStatus | "")}
            className="w-full pl-9 pr-4 h-min py-2 bg-[#191621] text-[#bebebe] rounded border border-[rgba(14,210,247,0.08)] text-sm appearance-none"
          >
            {statuses.map(status => (
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {filteredRecords.length === 0 && (
          <div className="flex items-center justify-center h-32 text-xs text-[#45aaff] opacity-50">
