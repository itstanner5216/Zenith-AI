    ...
    <div className="flex flex-col gap-3 p-4 mx-auto max-w-xl">
      {displayedExamples.map((example, index) => (
        <button
          ...
          className="text-left p-3 border border-[var(--border-defined)] hover:border-[var(--border-accent)] bg-[var(--bg-depth-3)] hover:bg-[var(--bg-depth-4)] shadow-elevation-md hover:shadow-[0_4px_14px_rgba(0,0,0,0.5),var(--glow-cyan-sm)] hover:translate-y-[-1px] active:scale-[0.98] transition-all duration-150 flex items-start gap-3 group w-full rounded-md cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--border-active)] focus-visible:ring-offset-1 focus-visible:ring-offset-[var(--bg-depth-2)]"
        >
          <div className="text-[var(--text-dim)] group-hover:text-[var(--text-accent)] text-xl transition-all duration-150 group-hover:drop-shadow-[0_0_4px_rgba(14,210,247,0.3)]">
            {example.icon}
          </div>
