    ...
    <div className="relative">
      <div
        className={`relative border-2 border-dashed  p-4 transition-colors cursor-pointer
          ${isDragging 
            ? 'border-neon-cyan bg-[rgba(14,210,247,0.04)]' 
            : 'border-defined hover:border-neon-cyan'
          }`}
      ...
      {error && (
        <div className="mt-2 text-sm text-neon-pink">
          {error}
        </div>
      )}
...
const AttachmentPreview: React.FC<{ attachment: LocalAttachment; onRemove: (id: string) => void }> = ({
  ...
  return (
    <div className="flex items-center justify-between p-2 bg-depth-3">
      <div className="flex items-center space-x-2">
        {attachment.contentType.startsWith('image/') ? (
          ...
          <div className="h-8 w-8 flex items-center justify-center bg-[var(--border-defined)] rounded">
            <svg
