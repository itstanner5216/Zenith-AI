...
export function ScreenpipeHandler({
  ...
  return (
    <div className="text-sm p-2">
      <div className="text-[var(--text-normal)] mb-2 font-medium">
      ...
      {isComplete && resultCount === 0 && !error && (
        <div className="text-xs text-[var(--text-dim)] mt-1">No results found</div>
      )}
      {!isComplete && !error && (
        <div className="text-xs text-[var(--text-dim)] mt-1 italic">
          {status || "Initializing..."}
        </div>
      )}
    </div>
