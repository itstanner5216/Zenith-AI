  ...
  <div className="flex items-center justify-between py-2.5 border-b border-[var(--border-subtle)] last:border-b-0 group hover:bg-[var(--background-modifier-hover)] rounded-md px-1 -mx-1 transition-colors duration-150">
    ...
    <div className="flex-shrink-0">
      <label className="relative inline-flex items-center cursor-pointer" title={value ? "Enabled" : "Disabled"}>
        ...
        <div className={`relative w-9 h-5 rounded-full border transition-all duration-200 ${value ? "bg-[var(--interactive-accent)] border-[var(--interactive-accent)] shadow-[0_0_6px_var(--interactive-accent)]" : "bg-[var(--background-modifier-form-field)] border-[var(--background-modifier-border)] group-hover:border-[var(--interactive-accent)]"}`}>
          <div className={`absolute top-0.5 w-3.5 h-3.5 rounded-full transition-all duration-200 shadow-sm ${value ? "translate-x-[18px] bg-[var(--background-primary)] shadow-[0_0_6px_var(--interactive-accent)]" : "translate-x-0.5 bg-[var(--text-muted)] opacity-70"}`} />
        </div>
      </label>
    </div>
  </div>
...
export const TextInputSetting: React.FC<TextInputSettingProps> = ({ name, description, value, placeholder, onChange }) => (
  <div className="py-2">
    ...
    <input type="text" value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} className="w-full px-3 py-2 text-xs rounded-md bg-[var(--bg-depth-1)] text-[var(--text-normal)] border border-[var(--border-defined)] focus:outline-none focus:border-[var(--interactive-accent)] focus:ring-1 focus:ring-[var(--interactive-accent)] transition-all duration-150 placeholder:text-[var(--text-dim)] placeholder:opacity-60" />
  </div>
);
...
export const DropdownSetting: React.FC<DropdownSettingProps> = ({ name, description, value, options, onChange }) => (
  <div className="flex items-center justify-between py-2.5 border-b border-[var(--border-subtle)] last:border-b-0 group hover:bg-[var(--background-modifier-hover)] rounded-md px-1 -mx-1 transition-colors duration-150">
    ...
    <div className="flex-shrink-0">
      <select value={value} onChange={(e) => onChange(e.target.value)} className="px-3 py-1.5 text-xs rounded-md bg-[var(--bg-depth-1)] text-[var(--text-normal)] border border-[var(--border-defined)] focus:outline-none focus:border-[var(--interactive-accent)] focus:ring-1 focus:ring-[var(--interactive-accent)] transition-all duration-150 appearance-none cursor-pointer">
